...
[`mosna.io`][mosna.io] layer uses [Polars](https://pola.rs) under the hood but
always hands back ordinary `pandas.DataFrame` objects, so nothing downstream
changes.
...
!!! note "Why Parquet reads stay on pandas"
...
See [`mosna.modeling`][mosna.modeling] for the full set of estimators and helpers.
