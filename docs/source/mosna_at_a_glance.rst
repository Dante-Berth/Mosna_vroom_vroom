...
#. Which cell types preferentially sit next to which (assortativity / mixing)?
#. What does each cell's local neighborhood look like (niches)?
#. Do those spatial features predict a clinical outcome (survival, response)?
...
.. list-table::
   ...
   * - Module
     - Responsibility
   * - :mod:`mosna.io`
     - ⚡ Fast Polars / Parquet I/O helpers (always return ``pandas.DataFrame``).
   * - :mod:`mosna.utils`
     - Small array / data helpers (``to_numpy``, ``renormalize``).
   * - :mod:`mosna.testnets`
     - Synthetic test networks (``make_*_net``) used in tests and demos.
   * - :mod:`mosna.preprocessing`
     - Transform / aggregate / batch-correct node tables.
   * - :mod:`mosna.assortativity`
     - Mixing matrices and assortativity statistics.
   * - :mod:`mosna.neighbors`
     - k-order neighbor aggregation statistics (NAS).
   * - :mod:`mosna.features`
     - Spatial-omic feature / niche computation.
   * - :mod:`mosna.clustering`
     - Dimensionality reduction, clustering, cluster post-processing.
   * - :mod:`mosna.modeling`
     - Survival / regression / classification / risk statistics.
   * - :mod:`mosna.plotting`
     - Visualisation helpers.
   * - :mod:`mosna.screening`
     - Parameter screening for niche detection.
...
* **Fast I/O** (:mod:`mosna.io`): Polars/Parquet for the slow paths — CSV read
...
See :doc:`how_to_guides` for usage and :doc:`theoretical_concepts` for the math.
